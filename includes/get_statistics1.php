...
$rows = query("SELECT symbol, indicator, min(date) min_date, max(date) max_date FROM `statistics` WHERE symbol=? group by symbol,indicator order by symbol,indicator","JD.L");
foreach($rows as $row){
	$symbol=$row['symbol'];
